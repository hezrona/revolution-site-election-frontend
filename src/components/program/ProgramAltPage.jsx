...
export default function ProgramAltPage() {
  const { data, loading } = useProgramContent();
  const content = data?.content || {};
  const sections =
    content.sections && content.sections.length ? content.sections : programSections;

  return (
    <main className="program-alt-page" id="program-alt-page">
      <ProgramHero content={content.hero} loading={loading} />
      <section className="program-alt-sections">
        <div className="container">
          {sections.map((section) => (
            <article className="program-alt-card" key={section.id}>
              <div className="program-alt-top">
                <div className="program-alt-media">
                  <img src={section.image} alt={section.imageAlt} />
                </div>
                <div className="program-alt-content">
                  <h2>{section.title}</h2>
                  <p className="program-alt-tag">{section.summaryTitle}</p>
                  <p className="program-alt-summary">{section.summary}</p>
                  <div className="program-alt-solutions">
                    {section.solutions?.map((solution, index) => (
                      <div className="program-alt-solution" key={solution.title}>
                        <span className="program-alt-index">{index + 1}</span>
                        <div>
              ...
              <figure className="program-alt-wide">
                <img src={section.image} alt={section.imageAlt} />
